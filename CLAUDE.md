...
**CI:** `.github/workflows/refresh-and-deploy.yml` runs on two crons: hourly at `:02` (regular score refresh + FIFA-only highlight fallback), and once nightly at `19:02 UTC` (~21:00 CEST) with `FORCE_YOUTUBE_FETCH=true` env passed to the highlight resolver so it burns YouTube quota exactly once per 24h. Also fires on push to `main` and on manual dispatch. It refreshes scores → resolves any new highlights (YouTube if the gate is on, FIFA always) → commits data + flag-reset (if any changed) as `github-actions[bot]` → builds → deploys to GitHub Pages. Secrets: `FOOTBALL_DATA_KEY` and `YOUTUBE_API_KEY`. The auto-commit uses `GITHUB_TOKEN`, so it doesn't retrigger the workflow.
